...
export function ConnectionsSettings() {
  ...
  const handleRefresh = async (connectionId: string) => {
    ...
    setConnections((prev) =>
      prev.map((conn) =>
        conn.id === connectionId ? { ...conn, lastSync: 'just now' } : conn
      )
    )
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg bg-success/5 border border-success/20">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-foreground-subtle">
              {connections.length} bank account
              {connections.length !== 1 ? 's' : ''} connected and syncing
              properly
            </p>
          </div>
          <Badge
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>
                            Disconnect {connection.bankName}?
                          </AlertDialogTitle>
                          <AlertDialogDescription>
                            This will remove access to your{' '}
                            {connection.bankName} accounts and stop automatic
                            transaction syncing. You can reconnect at any time.
                            <div className="mt-4 p-3 rounded-lg bg-warning/10 border border-warning/20">
                              <div className="flex items-start gap-2">
                                <AlertCircle className="w-4 h-4 text-warning mt-0.5" />
                                <div className="text-sm">
                                  <strong>Important:</strong> Your historical
                                  transaction data will be preserved, but new
                                  transactions won't be imported until you
                                  reconnect.
                                </div>
                              </div>
                            </div>
