...
export function TokenWalletCard({ balance, address }: TokenWalletCardProps) {
  ...
  const handleCopyAddress = async () => {
    try {
      await navigator.clipboard.writeText(`0x742d35Cc6134C0532925a3b8D8C58F2L7ec47c9f`);
      setIsCopied(true);
    ...
    <Card variant="glass" className="relative overflow-hidden">
      ...
      <CardContent className="relative space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-sm text-foreground-subtle">Token Balance</span>
            <Button 
              variant="ghost" 
              size="sm" 
        ...
        <div className="grid grid-cols-2 gap-4 pt-2">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1">
              <span className="font-semibold">{formatBalance(monthlyEarnings)}</span>
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1">
              <span className="font-semibold">{formatBalance(totalEarnings)}</span>
              <span className="text-xs text-foreground-muted">SWT</span>
            </div>
        ...
        <div className="p-3 rounded-lg bg-primary/5 border border-primary/10">
          ...
          <p className="text-xs text-foreground-subtle">
            Use SWT tokens to unlock premium features, access exclusive financial courses, 
            or exchange for real rewards and discounts.
          </p>
        </div>
      </CardContent>
    </Card>
